...
public class StatusBarUtil {
    ...
    public static void setColorForSwipBack(Activity activity, @ColorInt int color) {
        setColorForSwipBack(activity,color,DEFAULT_STATUS_BAR_ALPHA);
    }
    @SuppressLint("ObsoleteSdkInt")
    public static void setColorForSwipBack(Activity activity, @ColorInt int color,@IntRange(from = 0,to = 255) int statusBarAlpha) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT){
//            ??
            ViewGroup contetView = ((ViewGroup) activity.findViewById(android.R.id.content));
            View rootView = contetView.getChildAt(0);
            int statusBarHeight = getStatusBarHeight(activity);
            if (rootView instanceof CoordinatorLayout) {
                final CoordinatorLayout coordinatorLayout = (CoordinatorLayout) rootView;
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP){
                    coordinatorLayout.setFitsSystemWindows(false);
                    contetView.setBackgroundColor(calculateStatusColor(color,statusBarAlpha));
                    boolean isNeedRequestLayout = contetView.getPaddingTop() < statusBarHeight;
                    if (isNeedRequestLayout) {
                        contetView.setPadding(0,statusBarHeight,0,0);
